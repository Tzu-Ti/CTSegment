...
class Saving():
    ...
    def configure_seg(self):
        pred = self.preds.permute(1, 2, 0).cpu().numpy()
        pred = self.mapping_back(pred)
        return pred
    
def save_niigz(ct, seg, default_orient, name, saving_folder):
    ...
    seg = nib.Nifti1Image(seg, None, header=ct_header)
    os.makedirs(saving_folder, exist_ok=True)
    ct.to_filename(os.path.join(saving_folder, '{}_CT.nii.gz'.format(name)))
    seg.to_filename(os.path.join(saving_folder, '{}_Seg.nii.gz'.format(name)))
