__author__ = 'Titi Wei'
from data.dataset import CTDataset, PredictDataset
...
def parser():
    ...
    # checkpoint path
    parser.add_argument('--ckpt_path', required='--test' in sys.argv or '--predict' in sys.argv)
...
class ModelFactory():
    def __init__(self, args, cfg):
        ...
        if args.train or args.fast_run:
            Dataset = CTDataset(folder=cfg['folder'], list_path=cfg['list_path'], num_classes=cfg['num_classes'], train=True)
            self.loader = DataLoader(Dataset, batch_size=cfg['batch_size'], shuffle=True, num_workers=cfg['num_workers'])
            Dataset = CTDataset(folder=cfg['folder'], list_path=cfg['val_list_path'], num_classes=cfg['num_classes'], train=False)
            self.val_loader = DataLoader(Dataset, batch_size=1, shuffle=False, num_workers=cfg['num_workers'])
        elif args.test:
            Dataset = CTDataset(folder=cfg['folder'], list_path=cfg['test_list_path'], num_classes=cfg['num_classes'], train=False)
            self.test_loader = DataLoader(Dataset, batch_size=self.cfg['batch_size'], shuffle=False, num_workers=cfg['num_workers'])
        elif args.predict:
            Dataset = PredictDataset(folder=args.patient_path, num_classes=cfg['num_classes'])
            self.predict_loader = DataLoader(Dataset, batch_size=self.cfg['batch_size'], shuffle=False, num_workers=cfg['num_workers'])
            self.S = tools.Saving(json_path=cfg['json_path'])
